...

{
    ...
    public function getPaymentMethodChargeMode($paymentMethod)
    {
        ...
        try {
            return $moduleSettingService->get($configKey, Constants::MODULE_ID);
        } catch (\Exception $e) {
            return '';
        }
    }
    ...
    public function getTransactions($payment_id, $order, $unzer = null)
    {
        ...
        if ($paymentBaseMethod == 'ppy') {
            $canCancel = $payment->getAmount()->getCharged() == 0 && $payment->getAmount()->getRemaining() > 0;
        } else {
